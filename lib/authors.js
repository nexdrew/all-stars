...
function addMaintainers (persons, maintainers, opts) {
  ...
  for (let m of maintainers) {
    ...
    addElementIfUnique(persons, npmUser, npmUser, 'emails', m.email, false)
  }
}

function addElementIfUnique (persons, user, npmUser, field, value, prepend) {
  if (!persons[user]) {
    ...
    if (prepend) persons[user][field].unshift(value)
    else persons[user][field].push(value)
  }
}
  ...
  if (opts.aliases && opts.aliases.github) {
    for (let user of Object.keys(opts.aliases.github)) {
      for (let alias of opts.aliases.github[user]) {
        addElementIfUnique(persons, user, user, 'githubUsers', alias, false)
      }
    }
  }

  // add any email aliases
  if (opts.aliases && opts.aliases.emails) {
    for (let user of Object.keys(opts.aliases.emails)) {
      for (let email of opts.aliases.emails[user]) {
        addElementIfUnique(persons, user, user, 'emails', email, false)
      }
    }
  }

  // add any name aliases
  if (opts.aliases && opts.aliases.names) {
    for (let user of Object.keys(opts.aliases.names)) {
      for (let name of opts.aliases.names[user]) {
        addElementIfUnique(persons, user, user, 'names', name, false)
      }
    }
  }
...
function squashAlias (persons, alias, user, opts) {
  ...
  if (!persons[user]) {
    ...
    persons[user].npmUsers.unshift(user)
  } else {
    ...
    .then(response => {
      ...
      if (items) {
        for (let i of Object.keys(items)) {
          ...
          if (fullname) addElementIfUnique(persons, user, npmUser, 'names', fullname, true)
        }
      }
      ...
      if (items) {
        for (let i of Object.keys(items)) {
          ...
          if (githubUser) addElementIfUnique(persons, user, npmUser, 'githubUsers', githubUser, false)
        }
      }
      ...
      //     if (email) addElementIfUnique(persons, user, npmUser, 'emails', String(email).toLowerCase(), false)
      //   }
      // }
      ...
      if (items) {
        for (let i of Object.keys(items)) {
          ...
          if (twitter) addElementIfUnique(persons, user, npmUser, 'twitters', twitter, false)
        }
      }
    ...
    .then(ghuser => {
      ...
      if (validate) {
        ...
        if (ghuser.email && stringInArray(ghuser.email, persons[user].emails)) {
          addGithubUser = true
          if (ghuser.name) addElementIfUnique(persons, user, npmUser, 'names', ghuser.name, false)
        }
        if (ghuser.name && stringInArray(ghuser.name, persons[user].names)) {
          addGithubUser = true
          if (ghuser.email) addElementIfUnique(persons, user, npmUser, 'emails', String(ghuser.email).toLowerCase(), true)
        }
        if (addGithubUser) addElementIfUnique(persons, user, npmUser, 'githubUsers', githubUser, false)
      } else {
        if (ghuser.name) addElementIfUnique(persons, user, npmUser, 'names', ghuser.name, false)
        if (ghuser.email) addElementIfUnique(persons, user, npmUser, 'emails', String(ghuser.email).toLowerCase(), true)
      }
